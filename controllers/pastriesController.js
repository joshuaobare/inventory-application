...
exports.pastry_update_get = asyncHandler(async (req, res, next) => {
    const pastry = await Pastries.findById(req.params.id).exec()
    res.render("create_form", {title: "Create Pastry", product: pastry})
});

// Handle pastry update on POST.
exports.pastry_update_post = [
    ...
    asyncHandler(async(req,res,next) => {
        ...
        if(!errors.isEmpty()){
            ...
            const updatedProduct = await Pastries.findByIdAndUpdate(req.params.id, pastry, {})
